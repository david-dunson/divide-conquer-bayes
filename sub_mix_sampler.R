##' Data augmentation for posterior sampling in finite mixture of Gaussians in subsets
##'
...
##'     after stochastic approximation. This modifies the standard data
##'     augmentation algorithm by raising the likelihood to the power of
##'     'nrep', which is $k$. See Srivastava et al. (2015) for details.
...
mvnSubMix <- function (dataMat, ncomp = 2, nrep = 10, niter = 10000, nburn = 5000, nthin = 5) {
    library(matrixStats)
    library(mvtnorm)
    library(MCMCpack)
